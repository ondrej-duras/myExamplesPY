...
OLDWI=WIDTH=640; OLDHEI=HEIGHT=480; FULL=0
...
win = pygame.display.set_mode((WIDTH,HEIGHT),RESIZABLE,32)
...
while True:
  ...
  for event in pygame.event.get():
    ...
    if (event.type == KEYDOWN) and (event.key == 113):  # 'q'
      done()
    if (event.type == KEYDOWN) and (event.key == 102):  # 'f'

      if FULL:   # from FullScreen into Resizable
        WIDTH=OLDWI; HEIGHT=OLDHEI; FULL=0
        win = pygame.display.set_mode((WIDTH,HEIGHT),RESIZABLE)
        pygame.display.set_caption("Polygon Q")
        poly = polygon(7)

      else:      # from Resizable into FullScreen
        OLDWI=WIDTH; OLDHEI=HEIGHT; FULL=1
    ...
    if event.type == VIDEORESIZE:  # Resizing window
